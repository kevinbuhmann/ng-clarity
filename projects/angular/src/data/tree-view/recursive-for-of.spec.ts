...
export default function (): void {
  describe('RecursiveChildren Component', function () {
    ...
    let clarityDirective: ClrRecursiveForOf<TestNode>;
    ...
    beforeEach(function (this: Context) {
      ...
      clarityDirective = fixture.componentInstance.recursiveForOf;
      featuresService = TestBed.inject(TreeFeaturesService);
    });

    it('accepts a [clrRecursiveForOf] input', function (this: Context) {
      expect(clarityDirective.nodes).toBe(TEST_ROOT);
    });

    it('accepts a [clrRecursiveForOfGetChildren] input', function (this: Context) {
      expect(clarityDirective.getChildren).toBe(getChildren);
    });

    it('initializes the recursion service with the correct values', function (this: Context) {
      expect(featuresService.recursion).toBeTruthy();
      expect(featuresService.recursion.template.elementRef.nativeElement).toBe(
        hostComponent.template.elementRef.nativeElement
      );
      expect(featuresService.recursion.root).toEqual([
        new RecursiveTreeNodeModel(TEST_ROOT, null, getChildren, featuresService),
      ]);
    });

    it('updates the recursion service if the nodes change', function (this: Context) {
      hostComponent.root = TEST_ROOT.children[0];
      fixture.detectChanges();
      expect(featuresService.recursion.root).toEqual([
        new RecursiveTreeNodeModel(TEST_ROOT.children[0], null, getChildren, featuresService),
      ]);
    });

    it('accepts multiple roots', function (this: Context) {
      hostComponent.root = [TEST_ROOT, TEST_ROOT];
      fixture.detectChanges();
      const wrappedRoot = new RecursiveTreeNodeModel(TEST_ROOT, null, getChildren, featuresService);
      expect(featuresService.recursion.root).toEqual([wrappedRoot, wrappedRoot]);
    });
  });
}
